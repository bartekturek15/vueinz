<template>
    <h1 class="display-6 text-light text-center mt-5">Wybierz przeciwnika, którego chcesz przejrzeć:</h1>
        ...
        <table class="table custom-table table-dark mx-auto">
            <thead>
                <tr>
                    ...
                    <th scope="col">Stopień wyzwania</th>
                    <th scope="col">Polubienia</th>
                </tr>
            </thead>
            <tbody v-if="this.items.length > 0">
                    <tr v-for="item in VisiblePost" :key="item.enemyId" class="position-relative">
                        <td scope="row">
                            <router-link class="stretched-link" style="text-decoration: none; color:whitesmoke" 
                            :to="{ name: 'ByMonsterId', params: {  id: item.enemyId, link: '/StworzeniPrzeciwnicy', doDelete: true  }}">
                                {{ item.ownerName }}
                            </router-link>
                        </td> 
                        <td>{{ item.enemyName }}</td> 
                        <td>{{ item.enemySize }}</td> 
                        <td>{{ item.enemyDangerLvl }}</td> 
                        <td>{{ item.upvotes }}</td>
                    </tr>
            </tbody>
            <tbody v-else>
...
<script>
   ...
   data() {
    ...
    computed: {
        ...
        filteredItems() {
            ...
            if (this.searchFilter !== '') {
                ...
                item.enemyName.toLowerCase().includes(this.searchFilter.toLowerCase()));
            }
            return newArr;
        },
        VisiblePost() {
    ...
    mounted() {
        axios.get('https://localhost:7095/api/Enemy/owner/' + this.user.userID, {
                headers: {
