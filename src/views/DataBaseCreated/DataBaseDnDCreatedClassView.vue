<template>
    <h1 class="display-6 text-light text-center mt-5">Wybierz klase, którą chcesz przejrzeć:</h1>
        ...
        <table class="table custom-table table-dark mx-auto">
            <thead>
                <tr>
                    ...
                    <th scope="col">Rzuty obronne</th>
                    <th scope="col">Kość życia</th>
                    <th scope="col">Polubienia</th>
                </tr>
            </thead>
            <tbody v-if="this.items.length > 0">
                    <tr v-for="item in VisiblePost" :key="item.classId" class="position-relative">
                        <td scope="row">
                            <router-link class="stretched-link" style="text-decoration: none; color:whitesmoke" 
                            :to="{ name: 'ByClassId', params: { id: item.classId, link: '/StworzoneKlasy', doDelete: true }}">
                                {{ item.ownerName }}
                            </router-link>
                        </td> 
                        <td>{{ item.className }}</td>  
                        <td>{{ item.classSavingThrows }}</td>
                        <td>{{ item.classHitDice }}</td>
                        <td>{{ item.upvotes }}</td>
                    </tr>
            </tbody>
            <tbody v-else>
...
<script>
   ...
   data() {
    ...
    computed: {
        ...
        filteredItems() {
            ...
            if (this.searchFilter !== '') {
                ...
                item.className.toLowerCase().includes(this.searchFilter.toLowerCase()));
            }
    ...
    mounted() {
        axios.get('https://localhost:7095/api/DndClass/owner/' + this.user.userID, {
                headers: {
