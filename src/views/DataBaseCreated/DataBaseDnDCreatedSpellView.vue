<template>
    <h1 class="display-6 text-light text-center mt-5">Wybierz czar, który chcesz przejrzeć:</h1>
    ...
    <div style="color: white;" class="container mt-4">
        <table class="table custom-table table-dark mx-auto">
            ...
            <tbody v-if="this.items.length > 0">
                    <tr v-for="item in VisiblePost" :key="item.spellID" class="position-relative">
                        <td v-if="item != null" scope="row">
                            <router-link class="stretched-link" style="text-decoration: none; color:whitesmoke" 
                            :to="{ name: 'BySpellId', params: { id: item.spellId, link: '/StworzoneCzary', doDelete: true }}">
                                {{ item.ownerName }}
                            </router-link>
                        </td> 
                        <td v-if="item != null">{{ item.spellName }}</td> 
                        <td v-if="item != null">{{ item.spellLevel }}</td> 
                        <td v-if="item != null">{{ item.spellSchool }}</td>  
                        <td v-if="item != null">{{ item.spellCasting }}</td>
                        <td v-if="item != null">{{ item.upvotes }}</td>
                    </tr>
            </tbody>
            <tbody v-else>
                <tr>
...
<script>
   ...
   data() {
    ...
    computed: {
        ...
        filteredItems() {
            ...
            if (this.searchFilter !== '') {
                ...
                item.spellName.toLowerCase().includes(this.searchFilter.toLowerCase()) && item != null);
            }
    ...
    mounted() {
        axios.get('https://localhost:7095/api/Spell/owner/' + this.user.userID, {
                headers: {
