<template>
    <h1 class="display-6 text-light text-center mt-5">Wybierz przedmiot, który chcesz przejrzeć:</h1>
        ...
        <table class="table custom-table table-dark mx-auto">
            <thead>
                <tr>
                    ...
                    <th scope="col">Rzadkość</th>
                    <th scope="col">Waga</th>
                    <th scope="col">Polubienia</th>
                </tr>
            </thead>
            <tbody v-if="this.items.length > 0">
                    <tr v-for="item in VisiblePost" :key="item.itemId" class="position-relative">
                        <td scope="row">
                            <router-link class="stretched-link" style="text-decoration: none; color:whitesmoke" 
                            :to="{ name: 'ByItemId', params: { id: item.itemId, link: '/StworzonePrzedmioty', doDelete: true  }}">
                                {{ item.ownerName }}
                            </router-link>
                        </td> 
                        <td>{{ item.itemName }}</td> 
...
<script>
   ...
   data() {
    ...
    mounted() {
        axios.get('https://localhost:7095/api/Item/owner/' + this.user.userID, {
                headers: {
