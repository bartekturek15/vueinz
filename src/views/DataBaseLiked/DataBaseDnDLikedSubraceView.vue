<template>
    <h1 class="display-6 text-light text-center mt-5">Wybierz podrase, którą chcesz przejrzeć:</h1>
            ...
            <tbody v-if="this.items.length > 0">
                    <tr v-for="item in VisiblePost" :key="item.raceId" class="position-relative">
                        <td v-if="item.inheritedRaceID != 0" scope="row">
                            ...
                            :to="{ name: 'ByRaceId', params: { id: item.raceId, link: '/PolubionePodrasy' }}">
                                {{ item.ownerName }}
                            </router-link>
                        </td> 
...
<script>
   ...
   data() {
    ...
    mounted() {
        axios.get('https://localhost:7095/api/Race/upvoted', {
                headers: {
